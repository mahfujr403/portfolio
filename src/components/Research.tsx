        ...
        <div className="space-y-10">
          ...
          <div className="relative min-h-[600px]">
            ...
            <div className={`grid md:grid-cols-2 lg:grid-cols-3 gap-6 transition-all duration-700 ease-in-out ${
              ...
              currentProjects.length < 3 ? 'justify-items-center' : ''
            }`}
            key={`current-${currentPage}`}
            style={{
              gridTemplateColumns: currentProjects.length < 3 && currentProjects.length > 0
                ? `repeat(${currentProjects.length}, minmax(0, 400px))`
                : undefined,
              justifyContent: currentProjects.length < 3 ? 'center' : undefined
            }}>
              ...
              >
                ...
                <div className="relative p-5 flex-1 flex flex-col justify-between space-y-4">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-lg font-bold text-foreground leading-tight transition-colors duration-300 group-hover/card:text-primary line-clamp-3">
                      {project.title}
                    </h3>
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-2 text-xs pt-2 border-t border-border/50">
                    {project.publisher && (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-muted-foreground">
                          <span className="font-semibold text-foreground">Publisher:</span> {project.publisher}
                        </span>
                      </div>
                    )}
                    {project.publicationType && (
                      <div className="flex items-center gap-1.5">
                        <div className="w-1.5 h-1.5 rounded-full bg-primary"></div>
                        <span className="text-muted-foreground">
                          <span className="font-semibold text-foreground">Type:</span> {project.publicationType}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    className="relative mt-2 w-full justify-between text-foreground bg-secondary/50 border border-border/50 hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all duration-300 overflow-hidden group/btn"
                  >
            ...
            {isTransitioning && (
              <div className={`absolute top-0 left-0 right-0 grid md:grid-cols-2 lg:grid-cols-3 gap-6 z-10 opacity-0 ${
                nextProjects.length < 3 ? 'justify-items-center' : ''
              }`}
              key={`next-${nextPage}`}
              style={{
                animation: 'zoomIn 700ms ease-in-out forwards',
                gridTemplateColumns: nextProjects.length < 3 && nextProjects.length > 0
                  ? `repeat(${nextProjects.length}, minmax(0, 400px))`
                  : undefined,
                justifyContent: nextProjects.length < 3 ? 'center' : undefined
              }}>
